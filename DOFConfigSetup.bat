...
if /i "%~1"=="--dof-config" (
    if "%~2"=="" (
        ...
        echo Example: DOFConfigSetup.bat --dof-config "C:\DirectOutput\Config"
        call :failExit 1
    )
    ...
    echo Usage: DOFConfigSetup.bat [--dof-config "X:\DirectOutput\Config"]
    call :failExit 1
)

echo.
echo ===== DOF Config Setup =====
echo.
...
set "copyLog=%TEMP%\DOFConfigSetup_%RANDOM%_%RANDOM%.log"
echo Copying files...
...
call :copyInstallArtifacts
if errorlevel 1 call :failExit 1

echo.
call :printInstallArtifactSummary
echo Success: Template files and DOF runtime files were refreshed.
...
echo - If Windows blocked this script, right-click it and choose Run anyway (then rerun DOFConfigSetup.bat).
...
set "backupLog=%TEMP%\DOFConfigSetup_backup_%RANDOM%_%RANDOM%.log"
...
for %%D in ("%dofInstallDir%\Config" "%dofInstallDir%\x64" "%dofInstallDir%\x86") do (
    if not exist "%%~D\" (
        mkdir "%%~D" >nul 2>&1
        if errorlevel 1 (
            echo Error: Could not create required folder "%%~D".
            echo Hint: Try running this script as Administrator.
            exit /b 1
        )
    )
...
:resolveInstallDir
REM Derive the DOF install directory from the selected Config folder path.
set "dofInstallDir=%selectedDestination%"
if /i "!dofInstallDir:~-7!"=="\Config" (
    set "dofInstallDir=!dofInstallDir:~0,-7!"
) else (
    echo Error: Destination does not end in \Config, so install folder cannot be derived safely.
    echo Selected destination: "%selectedDestination%"
    echo Hint: Choose the DOF Config folder, for example C:\DirectOutput\Config.
    exit /b 1
)

if not defined dofInstallDir (
    echo Error: Unable to determine DOF install directory from "%selectedDestination%".
    exit /b 1
)
exit /b 0
...
if not exist "!singleSource!" (
    echo Error: Missing package file for !singleLabel!.
    echo Expected source: "!singleSource!"
    exit /b 1
)

copy /y "!singleSource!" "!singleTarget!" >nul
if errorlevel 1 (
    echo Error: Failed to copy !singleLabel! to "!singleTarget!".
    echo Hint: Check permissions or run this script as Administrator.
    exit /b 1
)
exit /b 0
