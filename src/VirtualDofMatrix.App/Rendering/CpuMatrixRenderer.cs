...
public sealed class CpuMatrixRenderer : IMatrixRenderer
{
    ...
    {
        // Shared latest-frame slot keeps transport and compose loop decoupled without queue growth.
        lock (_frameGate)
        {
    ...
    private static MatrixConfig BuildConfig(int width, int height, DotStyleConfig dotStyleConfig)
    {
        return new MatrixConfig
        {
            ...
            MinDotSpacing = Math.Max(dotStyleConfig.DotSpacingX, dotStyleConfig.DotSpacingY),
            MinDotSpacingX = dotStyleConfig.DotSpacingX,
            MinDotSpacingY = dotStyleConfig.DotSpacingY,
