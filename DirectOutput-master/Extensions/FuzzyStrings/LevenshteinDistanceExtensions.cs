...
namespace FuzzyStrings
{
	public static class LevenshteinDistanceExtensions
	{
		...
		/// <returns></returns>
		public static double LevenshteinDistance(this string input, string comparedTo, bool caseSensitive = false)
		{
			if (string.IsNullOrWhiteSpace(input) || string.IsNullOrWhiteSpace(comparedTo)) return 0;
			if (!caseSensitive)
			{
				input = input.ToLower();
